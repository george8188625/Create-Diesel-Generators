...
public class CDGCommands {
    ...
    private int getOilChunk(CommandSourceStack source) throws CommandSyntaxException {
        if(!source.hasPermission(2))
            return 0;
        ChunkPos chunkPos = new ChunkPos(new BlockPos(source.getPosition()));
        ...
        source.sendSuccess(Components.literal("There is ").withStyle(ChatFormatting.GRAY).append(Components.literal(amount+"B").withStyle(ChatFormatting.GOLD)).append(" of Oil in this Chunk.").withStyle(ChatFormatting.GRAY), false);
    ...
    private int refreshOilChunk(CommandSourceStack source) throws CommandSyntaxException {
        ...
        source.sendSuccess(Components.literal("Refreshed this chunks oil contents").withStyle(ChatFormatting.GRAY), false);
    ...
    private int setOilChunk(CommandSourceStack source, CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        source.sendSuccess(Components.literal("Set this chunk's oil deposits to  ").withStyle(ChatFormatting.GRAY).append(Components.literal(amount+"B").withStyle(ChatFormatting.GOLD)), false);
...
//                ChunkPos chunkPos = new ChunkPos((int) (source.getPosition().x + x * 16)/16, (int) (source.getPosition().z + z * 16)/16);
        ...
        if(closestAmount.get() != 0) {
            ...
            source.sendSuccess(Components.literal("There is oil in the chunk ").withStyle(ChatFormatting.GRAY).append(Components.literal( finalX + " " + finalZ).withStyle(ChatFormatting.GOLD).withStyle(
                    a -> a.withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tp @s " + finalX*16 + " ~ " + finalZ*16)
                    ))).append(" with ").withStyle(ChatFormatting.GRAY).append(Components.literal(closestAmount + "B ").withStyle(ChatFormatting.GOLD)).append(Components.literal("of oil.").withStyle(ChatFormatting.GRAY)), false);
            return 1;
        }
        source.sendFailure(Components.literal("There is no oil chunk nearby").withStyle(ChatFormatting.RED));
        return 1;
    }
}
