...
public class DieselEngineCategory implements IRecipeCategory<DieselEngineJeiRecipeType> {
    ...
    public void draw(DieselEngineJeiRecipeType recipe, IRecipeSlotsView iRecipeSlotsView, PoseStack matrixStack, double mouseX, double mouseY) {
        ...
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.burnRate).component().append(Component.translatable("createdieselgenerators.generic.unit.mbps")).withStyle(ChatFormatting.BOLD), 81,
                15, 0xaaaaaa);
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.burnRate).component().append(Component.translatable("createdieselgenerators.generic.unit.mbps")).withStyle(ChatFormatting.BOLD), 81,
                14, 0xeeeeee);
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.speed).component().append(Component.translatable("create.generic.unit.rpm")).withStyle(ChatFormatting.BOLD), 81,
                33, 0xaaaaaa);
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.stress).component().append(Component.translatable("create.generic.unit.stress")).withStyle(ChatFormatting.BOLD), 81,
                50, 0xaaaaaa);
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.speed).component().append(Component.translatable("create.generic.unit.rpm")).withStyle(ChatFormatting.BOLD), 80,
                32, 0xeeeeee);
        Minecraft.getInstance().font.draw(matrixStack, Lang.number(recipe.stress).component().append(Component.translatable("create.generic.unit.stress")).withStyle(ChatFormatting.BOLD), 80,
                49, 0xeeeeee);
