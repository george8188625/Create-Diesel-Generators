...
public class DistillationCategory extends CreateRecipeCategory<DistillationRecipe> {
    ...
    public void draw(DistillationRecipe recipe, IRecipeSlotsView iRecipeSlotsView, PoseStack matrixStack, double mouseX, double mouseY) {
        ...
        AllGuiTextures.JEI_ARROW.render(matrixStack, 40, 150);
        AllGuiTextures shadow = noHeat ? AllGuiTextures.JEI_SHADOW : AllGuiTextures.JEI_LIGHT;
        shadow.render(matrixStack, 81, 153 + (noHeat ? 10 : 30));
        distillationTower.draw(matrixStack, 91, 142, recipe.getFluidResults().size());

        if(!noHeat)
            heater.withHeat(requiredHeat.visualizeAsBlazeBurner()).draw(matrixStack, 91, 150);

        AllGuiTextures heatBar = noHeat ? AllGuiTextures.JEI_NO_HEAT_BAR : AllGuiTextures.JEI_HEAT_BAR;

        heatBar.render(matrixStack, 4, 170);
